...
import { createLessonContentQueryOptions } from "@/hooks/query-options"
...
export default function LessonViewer() {
  ...
  const { data:lesson, isPending:lessonLoading, error:lessonError} = useQuery(createLessonContentQueryOptions(Number(params.id)))
  const [currentSublesson, setCurrentSublesson] = useState(0)

  const sublessons: Sublesson[] | undefined = useMemo( () => {
    if (!lesson) return []
    return createSublessons(lesson.cards)
  }, [lesson])

  if (lessonLoading || !sublessons) {
    return (
